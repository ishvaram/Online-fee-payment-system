...
$row=mysql_fetch_array($result);
	...
	$query="select acno,balance,password,cvv from bank where acno=$accountnumber";
	$result=mysql_query("$query",$con);
	$row=mysql_fetch_array($result);
	if($row['acno']==$accountnumber && $row['balance']>=$amttbpaid && $row['password']==$_POST['pass'] && $row['cvv']==$_POST['cvv'])
	{
	echo '
...
<table style="font-size:26px; margin-left:300px;"  border="1" cellpadding="5" cellspacing="5" width="700">
	...
	<input type="submit" value="OKAY" style="font-size:20px;">
	</form>
	</div>';
	$query="update student set amtpaid=1 where rollno=$rollno5";
	mysql_query("$query",$con);
	$query="update bank set balance=balance-$amttbpaid where acno=$accountnumber";
	mysql_query("$query",$con);
	mysql_close($con);
	}
	else
	{
		header("location:studlogin1.php");
		mysql_close($con);
	}
	?>
